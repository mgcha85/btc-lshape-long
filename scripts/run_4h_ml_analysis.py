...
from crypto_backtest.data_loader import BacktestConfig, load_4h_data
...
def run_ml_analysis():
    ...
    print("\n" + "=" * 60)
    print("ML RESULTS (4-hour timeframe)")
    print("=" * 60)
    ...
    train_size = min(100, len(X) // 3)
    test_size = min(30, len(X) // 10)
    
    if len(X) >= train_size + test_size * 2:
    ...
    print("SUMMARY")
    print("=" * 60)
    print(f"  4h samples: {len(y):,}")
    print(f"  4h best AUC: {max(r.auc_roc for r in results):.4f}")
